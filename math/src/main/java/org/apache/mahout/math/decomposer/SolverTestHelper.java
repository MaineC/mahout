...
public abstract class SolverTestHelper {
  private static final Logger log = LoggerFactory.getLogger(SolverTestHelper.class);

  public static boolean isOrthonormal(Matrix eigens) {
    return isOrthonormal(eigens, 1.0e-6);
  }

  public static boolean isOrthonormal(Matrix currentEigens, double errorMargin) {
    ...
    for (int i = 0; i < currentEigens.numRows(); i++) {
      ...
      for (int j = 0; j <= i; j++) {
        ...
        if (i == j) {
          if (! (Math.abs(1.0 - dot) < errorMargin)) {
            log.error("not norm 1 : " + dot + " (eigen #" + i + ')');
            return false;
          }
        } else {
  ...
  public static boolean isOrthonormal(LanczosState state) {
  ...
  public static boolean isEigen(Matrix eigens, VectorIterable corpus, double errorMargin, boolean isSymmetric) {
    return isEigen(eigens, corpus, eigens.numRows(), errorMargin, isSymmetric);
  }

  public static boolean isEigen(Matrix eigens,
    ...
    for (int i = 0; i < numEigensToCheck; i++) {
      ...
      if (! isEigen(i, e, corpus, errorMargin, isSymmetric))
        return false;
    }
    return true;
  }

  public static boolean isEigen(int i, Vector e, VectorIterable corpus, double errorMargin,
      boolean isSymmetric) {
    ...
    if (! (Math.abs(error) < errorMargin)) {
      log.error("Error: {" + error + " too high! (for eigen " + i + ')');
      return false;
    }
    return true;
  }
