 ...
 * </ul>
 */
public final class ConditionalEntropy extends AbstractCLI {
  ...
  private void calculateSpecificConditionalEntropy() throws IOException, ClassNotFoundException, InterruptedException {
    ...
    job.getConfiguration().set(ConditionalEntropyConfig.NUMBER_ITEMS_PARAM, String.valueOf(numberItems));
    boolean succeeded = job.waitForCompletion(true);
    if (!succeeded) {
      throw new IllegalStateException("Job failed!");
    }
