...
import static org.apache.mahout.clustering.minhash.MinHashConfig.*;

public final class MinHashDriver extends AbstractCLI {
  ...
  private void runJob(Path input, 
    ...
    conf.setInt(MIN_CLUSTER_SIZE, minClusterSize);
    conf.setInt(MIN_VECTOR_SIZE, minVectorSize);
    conf.set(HASH_TYPE, hashType);
    conf.setInt(NUM_HASH_FUNCTIONS, numHashFunctions);
    conf.setInt(KEY_GROUPS, keyGroups);
    conf.setBoolean(DEBUG_OUTPUT, debugOutput);
