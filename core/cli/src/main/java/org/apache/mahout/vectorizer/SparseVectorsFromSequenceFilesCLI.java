...
public class SparseVectorsFromSequenceFilesCLI extends AbstractCLI {
  private static final Logger log = LoggerFactory
      .getLogger(SparseVectorsFromSequenceFilesCLI.class);

  public static void main(String[] args) throws Exception {
    ToolRunner.run(new SparseVectorsFromSequenceFilesCLI(), args);
  }
  ...
  public int run(String[] args) throws Exception {
    ...
    Option maxDFSigmaOpt = obuilder
        ...
        .withRequired(false)
        .withArgument(
            abuilder.withName("maxDFSigma").withMinimum(1).withMaximum(1)
                .create())
        .withDescription(
    ...
    Option numReduceTasksOpt = obuilder
        ...
        .withArgument(
            abuilder.withName("numReducers").withMinimum(1).withMaximum(1)
                .create())
        .withDescription("(Optional) Number of reduce tasks. Default Value: 1")
    ...
    Option maxNGramSizeOpt = obuilder
        ...
        .withRequired(false)
        .withArgument(
            abuilder.withName("ngramSize").withMinimum(1).withMaximum(1)
                .create())
        .withDescription(
    ...
    Group group = gbuilder.withName("Options").withOption(minSupportOpt)
        ...
        .withOption(weightOpt).withOption(powerOpt).withOption(minLLROpt)
        .withOption(numReduceTasksOpt).withOption(maxNGramSizeOpt)
        .withOption(overwriteOutput).withOption(helpOpt)
        .withOption(sequentialAccessVectorOpt).withOption(namedVectorOpt)
    ...
    try {
      ...
      config.setAnalyzerClass(analyzerClass);
      config.setSequentialAccessOutput(sequentialAccessOutput);
      config.setNamedVectors(namedVectors);
      config.setMaxDFSigma(maxDFSigma);
      config.setProcessIdf(processIdf);
      config.setMinLLRValue(minLLRValue);
      config.setNorm(norm);
      config.setLogNormalize(logNormalize);
      config.setReduceTasks(reduceTasks);
      config.setChunkSize(chunkSize);
      config.setMinSupport(minSupport);
      config.setMaxNGramSize(maxNGramSize);
      config.setMaxDFPercent(maxDFPercent);
      config.setMinDf(minDf);
      job.run(inputDir, outputDir, config);
      return 0;
