...
public class RecommenderJobTest extends MahoutTestCase {
  ...
  public void testCompleteJob() throws Exception {
    ...
    recommenderJob.setTempPath(new Path(tmpDir.getAbsolutePath()));
    RecommenderConfig config = new RecommenderConfig();
    RowSimilarityConfig rsc = config.getRowSimilarityConfig();
    rsc.setSimilarityClassName(TanimotoCoefficientSimilarity.class.getName());
    config.setNumRecommendations(4);

    recommenderJob.run(config);
  ...
  public void testCompleteJobBoolean() throws Exception {
    ...
    RecommenderConfig config = new RecommenderConfig();
    RowSimilarityConfig rsc = config.getRowSimilarityConfig();
    rsc.setSimilarityClassName(CooccurrenceCountSimilarity.class.getName());
    config.setBooleanData(true);
    config.setUsersFile(usersFile.getAbsolutePath());
  ...
  @Test
   public void testCompleteJobWithFiltering() throws Exception {
     ...
     RowSimilarityConfig rsc = config.getRowSimilarityConfig();
     rsc.setSimilarityClassName(TanimotoCoefficientSimilarity.class.getName());
     config.setNumRecommendations(1);
     config.setUsersFile(userFile.getAbsolutePath());
     config.setFilterFile(filterFile.getAbsolutePath());
