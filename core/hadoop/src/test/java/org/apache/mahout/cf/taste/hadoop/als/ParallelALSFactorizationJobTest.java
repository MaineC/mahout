...
public class ParallelALSFactorizationJobTest extends MahoutTestCase {
  ...
  public void completeJobToyExample() throws Exception {
    ...
    alsFactorization.setTempPath(new Path(tmpDir.getAbsolutePath()));
    HadoopUtil.delete(conf,  new Path(outputDir.getAbsolutePath()));
    alsFactorization.run(new Path(inputFile.getAbsolutePath()), new Path(outputDir.getAbsolutePath()),
        numIterations, numFeatures, lambda, 40, false);
  ...
  public void completeJobImplicitToyExample() throws Exception {
    ...
    HadoopUtil.delete(conf, new Path(outputDir.getAbsolutePath()));
    alsFactorization.run(new Path(inputFile.getAbsolutePath()), new Path(outputDir.getAbsolutePath()),
        numIterations, numFeatures, lambda, alpha, true);
