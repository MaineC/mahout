...
public class RowSimilarityJobTest extends MahoutTestCase {
  ...
  public void toyIntegration() throws Exception {
    ...
    rowSimilarityJob.setTempPath(new Path(tmpDir.getAbsolutePath()));
    RowSimilarityConfig config = new RowSimilarityConfig();
    config.setNumberOfCols(5);
    config.setSimilarityClassName(TanimotoCoefficientSimilarity.class.getName());
    rowSimilarityJob.run(config, new Path(inputFile.getAbsolutePath()), new Path(outputDir.getAbsolutePath()));
  ...
  public void toyIntegrationMaxSimilaritiesPerRow() throws Exception {
    ...
    rowSimilarityJob.setTempPath(new Path(tmpDir.getAbsolutePath()));
    RowSimilarityConfig config = new RowSimilarityConfig();
    config.setNumberOfCols(6);
    config.setSimilarityClassName(TanimotoCoefficientSimilarity.class.getName());
    config.setMaxSimilaritiesPerRow(1);
    config.setExcludeSelfSimilarity(true);
    rowSimilarityJob.run(config, new Path(inputFile.getAbsolutePath()), new Path(outputDir.getAbsolutePath()));
