...
public final class TestMeanShift extends MahoutTestCase {
  ...
  public void testCanopyEuclideanMRJob() throws Exception {
    ...
    driver.setTempPath(getTestTempDirPath("testdata"));
    driver.run(conf, input, output, new EuclideanDistanceMeasure(), new TriangularKernelProfile(), 4, 1, 0.2, 7, false, true, false);
  ...
  public void testCanopyEuclideanSeqJob() throws Exception {
    ...
    MeanShiftCanopyDriver driver = new MeanShiftCanopyDriver();
    driver.setTempPath(getTestTempDirPath("testdata"));
    driver.run(conf, input, output, new EuclideanDistanceMeasure(), new TriangularKernelProfile(), 4, 1, 0.2, 7, false, true, true);
  ...
  public void testCanopyEuclideanMRJobNoClustering() throws Exception {
    ...
    MeanShiftCanopyDriver driver = new MeanShiftCanopyDriver();
    driver.setTempPath(getTestTempDirPath("testdata"));
    driver.run(conf, input, output, new EuclideanDistanceMeasure(), new TriangularKernelProfile(), 4, 1, 0.2, 7, false, false, false);
  ...
  public void testCanopyEuclideanSeqJobNoClustering() throws Exception {
    ...
    MeanShiftCanopyDriver driver = new MeanShiftCanopyDriver();
    driver.setTempPath(getTestTempDirPath("testdata"));
    driver.run(conf, input, output, new EuclideanDistanceMeasure(), new TriangularKernelProfile(), 4, 1, 0.2, 7, false, false, true);
    Path outPart = new Path(output, "clusters-7-final/part-r-00000");
    long count = HadoopUtil.countRecords(outPart, conf);
    assertEquals("count", 3, count);
    Iterator<?> iterator = new SequenceFileValueIterator<Writable>(outPart,
