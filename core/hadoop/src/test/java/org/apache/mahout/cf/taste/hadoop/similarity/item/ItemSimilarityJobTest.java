...
public final class ItemSimilarityJobTest extends TasteTestCase {
  ...
  public void testCompleteJob() throws Exception {
    ...
    similarityJob.setTempPath(new Path(tmpDir.getAbsolutePath()));

    RecommenderConfig config = new RecommenderConfig();
    config.setBooleanData(false);
    config.setMaxPrefsPerUser(ItemSimilarityJob.DEFAULT_MAX_PREFS_PER_USER);
    config.setMinPrefsPerUser(ItemSimilarityJob.DEFAULT_MIN_PREFS_PER_USER);
    config.setMaxSimilaritiesPerItem(ItemSimilarityJob.DEFAULT_MAX_SIMILAR_ITEMS_PER_ITEM);
    config.getRowSimilarityConfig().setDefaults();
    config.getRowSimilarityConfig().setSimilarityClassName(CosineSimilarity.class.getName());
    config.getRowSimilarityConfig().setThreshold(RowSimilarityJob.NO_THRESHOLD);

    similarityJob.run(new Path(inputFile.getAbsolutePath()), new Path(outputDir.getAbsolutePath()),
        config);
  ...
  public void testMaxSimilaritiesPerItem() throws Exception {
    ...
    config.setMaxPrefsPerUser(ItemSimilarityJob.DEFAULT_MAX_PREFS_PER_USER);
    config.setMinPrefsPerUser(ItemSimilarityJob.DEFAULT_MIN_PREFS_PER_USER);
    config.setMaxSimilaritiesPerItem(1);
    config.getRowSimilarityConfig().setDefaults();
    config.getRowSimilarityConfig().setSimilarityClassName(TanimotoCoefficientSimilarity.class.getName());
    config.getRowSimilarityConfig().setThreshold(RowSimilarityJob.NO_THRESHOLD);
    similarityJob.setTempPath(getTestTempDirPath());
