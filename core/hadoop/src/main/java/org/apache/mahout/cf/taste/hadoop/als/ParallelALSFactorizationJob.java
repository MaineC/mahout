...
public class ParallelALSFactorizationJob extends AbstractJob {
  ...
  public int run(Path inputPath, Path outputPath,
      int numIterations, int numFeatures, double lambda, double alpha, boolean implicitFeedback
      ) throws IOException, InterruptedException, ClassNotFoundException {
    Path pathToUserRatings = new Path(outputPath, "userRatings");
    Path pathToM = new Path(outputPath, "M");
    Path pathToU = new Path(outputPath, "U");
   ...
   /* create A' */
    Job itemRatings = prepareJob(inputPath, pathToItemRatings(),
    ...
    /* create an initial M */
    initializeM(averageRatings, numFeatures, pathToM, numIterations);

    for (int currentIteration = 0; currentIteration < numIterations; currentIteration++) {
      /* broadcast M, read A row-wise, recompute U row-wise */
      log.info("Recomputing U (iteration {}/{})", currentIteration, numIterations);
      runSolver(pathToUserRatings, pathToU(currentIteration, numIterations, pathToU), pathToM(currentIteration - 1, numIterations, pathToM), implicitFeedback,
          lambda, alpha, numFeatures);
      /* broadcast U, read A' row-wise, recompute M row-wise */
      log.info("Recomputing M (iteration {}/{})", currentIteration, numIterations);
      runSolver(pathToItemRatings(), pathToM(currentIteration, numIterations, pathToM), pathToU(currentIteration, numIterations, pathToU), implicitFeedback,
          lambda, alpha, numFeatures);
    }
  ...
  private void initializeM(Vector averageRatings, int numFeatures, Path pathToM, int numIterations) throws IOException {
    Random random = RandomUtils.getRandom();

    FileSystem fs = FileSystem.get(pathToM.toUri(), getConf());
    SequenceFile.Writer writer = null;
    try {
      writer = new SequenceFile.Writer(fs, getConf(), new Path(pathToM(-1, numIterations, pathToM), "part-m-00000"), IntWritable.class,
          VectorWritable.class);
  ...
  private void runSolver(Path ratings, Path output, Path pathToUorI, boolean implicitFeedback,
      double lambda, double alpha, int numFeatures)
  ...
  private Path pathToM(int iteration, int numIterations, Path outputM) {
    return iteration == numIterations - 1 ? outputM : getTempPath("M-" + iteration);
  }

  private Path pathToU(int iteration, int numIterations, Path outputU) {
    return iteration == numIterations - 1 ? outputU : getTempPath("U-" + iteration);
  }
