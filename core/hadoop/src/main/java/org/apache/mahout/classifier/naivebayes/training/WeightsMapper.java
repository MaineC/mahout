...
//    instance.addTo(weightsPerFeature);
  ...
  protected void cleanup(Context ctx) throws IOException, InterruptedException {
    if (weightsPerFeature != null) {
      ctx.write(new Text(TrainNaiveBayesConfig.WEIGHTS_PER_FEATURE), new VectorWritable(weightsPerFeature));
      ctx.write(new Text(TrainNaiveBayesConfig.WEIGHTS_PER_LABEL), new VectorWritable(weightsPerLabel));
    }
    super.cleanup(ctx);
  }
}
