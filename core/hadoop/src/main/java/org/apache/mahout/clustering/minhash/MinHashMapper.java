...
public class MinHashMapper extends Mapper<Text, VectorWritable, Text, Writable> {
  ...
  protected void setup(Context context) throws IOException, InterruptedException {
    ...
    this.numHashFunctions = conf.getInt(MinHashConfig.NUM_HASH_FUNCTIONS, 10);
    this.minHashValues = new int[numHashFunctions];
    this.bytesToHash = new byte[4];
    this.keyGroups = conf.getInt(MinHashConfig.KEY_GROUPS, 1);
    this.minVectorSize = conf.getInt(MinHashConfig.MIN_VECTOR_SIZE, 5);
    String htype = conf.get(MinHashConfig.HASH_TYPE, "linear");
    this.debugOutput = conf.getBoolean(MinHashConfig.DEBUG_OUTPUT, false);
