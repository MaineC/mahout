...
public class ThetaMapper extends Mapper<IntWritable, VectorWritable, Text, VectorWritable> {
  ...
  protected void setup(Context ctx) throws IOException, InterruptedException {
    ...
    if (conf.getBoolean(TRAIN_COMPLEMENTARY, false)) {
      trainer = new ComplementaryThetaTrainer(scores.get(TrainNaiveBayesConfig.WEIGHTS_PER_FEATURE),
                                              scores.get(TrainNaiveBayesConfig.WEIGHTS_PER_LABEL), alphaI);
    } else {
      trainer = new StandardThetaTrainer(scores.get(TrainNaiveBayesConfig.WEIGHTS_PER_FEATURE),
                                         scores.get(TrainNaiveBayesConfig.WEIGHTS_PER_LABEL), alphaI);
    }
  }
  ...
  protected void cleanup(Context ctx) throws IOException, InterruptedException {
    ctx.write(new Text(TrainNaiveBayesConfig.LABEL_THETA_NORMALIZER),
