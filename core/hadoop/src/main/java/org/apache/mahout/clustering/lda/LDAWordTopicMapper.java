...
public class LDAWordTopicMapper extends Mapper<WritableComparable<?>,VectorWritable,IntPairWritable,DoubleWritable> {
  ...
  protected void map(WritableComparable<?> key,
    ...
    for (int k = 0; k < state.getNumTopics(); ++k) {
      IntPairWritable kw = new IntPairWritable(k, LDAConfig.TOPIC_SUM_KEY);
      v.set(logTotals[k]);
      assert !Double.isNaN(v.get());
      context.write(kw, v);
    }
    IntPairWritable llk = new IntPairWritable(LDAConfig.LOG_LIKELIHOOD_KEY, LDAConfig.LOG_LIKELIHOOD_KEY);
  ...
  public void configure(Configuration job) {
    LDAState myState = LDAConfig.createState(job);
    configure(myState);
  }
