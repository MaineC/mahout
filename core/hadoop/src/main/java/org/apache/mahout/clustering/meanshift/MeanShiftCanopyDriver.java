...
public class MeanShiftCanopyDriver extends AbstractJob {
  ...
  private static Path buildClustersMR(Configuration conf, Path clustersIn,
    ...
    while (!converged && iteration <= maxIterations) {
      int numReducers = Integer.valueOf(conf.get(MeanShiftConfig.MAPRED_REDUCE_TASKS, "1"));
      ...
      if (numReducers > 1) {
        numReducers--;
        conf.set(MeanShiftConfig.MAPRED_REDUCE_TASKS, String.valueOf(numReducers));
      }
    }
    Path fromPath = new Path(output, Cluster.CLUSTERS_DIR + (iteration-1));
  ...
  private static void clusterDataMR(Path input, Path clustersIn, Path output)
      throws IOException, InterruptedException, ClassNotFoundException {
    Configuration conf = new Configuration();
    conf.set(MeanShiftConfig.STATE_IN_KEY, clustersIn.toString());
    Job job = new Job(conf,
