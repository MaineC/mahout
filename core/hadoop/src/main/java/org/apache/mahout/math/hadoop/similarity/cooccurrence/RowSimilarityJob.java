...
public class RowSimilarityJob extends AbstractJob {
  ...
  public int run(RowSimilarityConfig conf, Path input, Path output) throws IOException, InterruptedException, ClassNotFoundException, InstantiationException, IllegalAccessException {
    return run(conf, input, output, null, null);
  }

  public int run(RowSimilarityConfig conf, Path input, Path output, Integer start, Integer end) throws IOException, InterruptedException, ClassNotFoundException, InstantiationException, IllegalAccessException {
    ...
    return run(conf, input, output, weightsPath, normsPath, numNonZeroEntriesPath, maxValuesPath, pairwiseSimilarityPath, start, end);
  }

  
  public int run(
      RowSimilarityConfig conf,
      Path input, Path output,
      Path weightsPath, Path normsPath, Path numNonZeroEntriesPath, Path maxValuesPath, Path pairwiseSimilarityPath,
      Integer start, Integer end) throws IOException, InterruptedException, ClassNotFoundException, InstantiationException, IllegalAccessException {

    if (conf.getNumberOfCols() == null) {
      conf.setNumberOfCols(getDimensions(input));
    }
    AtomicInteger currentPhase = new AtomicInteger();

    if (shouldRunNextPhase(start, end, currentPhase)) {
      Job normsAndTranspose = prepareJob(input, weightsPath, VectorNormMapper.class, IntWritable.class,
      ...
      normsAndTransposeConf.set(THRESHOLD, String.valueOf(conf.getThreshold()));
      ...
      normsAndTransposeConf.set(SIMILARITY_CLASSNAME, conf.getSimilarityClassName());
    ...
    if (shouldRunNextPhase(start, end, currentPhase)) {
      ...
      pairwiseConf.set(THRESHOLD, String.valueOf(conf.getThreshold()));
      ...
      pairwiseConf.set(SIMILARITY_CLASSNAME, conf.getSimilarityClassName());
      pairwiseConf.setInt(NUMBER_OF_COLUMNS, conf.getNumberOfCols());
      pairwiseConf.setBoolean(EXCLUDE_SELF_SIMILARITY, conf.getExcludeSelfSimilarity());
    ...
    if (shouldRunNextPhase(start, end, currentPhase)) {
      Job asMatrix = prepareJob(pairwiseSimilarityPath, output, UnsymmetrifyMapper.class,
      ...
      asMatrix.getConfiguration().setInt(MAX_SIMILARITIES_PER_ROW, conf.getMaxSimilaritiesPerRow());
