...
public class CachingCVB0PerplexityMapper extends
  ...
  protected void setup(Context context) throws IOException, InterruptedException {
    ...
    float eta = conf.getFloat(CVB0Config.TERM_TOPIC_SMOOTHING, Float.NaN);
    float alpha = conf.getFloat(CVB0Config.DOC_TOPIC_SMOOTHING, Float.NaN);
    long seed = conf.getLong(CVB0Config.RANDOM_SEED, 1234L);
    random = RandomUtils.getRandom(seed);
    numTopics = conf.getInt(CVB0Config.NUM_TOPICS, -1);
    int numTerms = conf.getInt(CVB0Config.NUM_TERMS, -1);
    int numUpdateThreads = conf.getInt(CVB0Config.NUM_UPDATE_THREADS, 1);
    int numTrainThreads = conf.getInt(CVB0Config.NUM_TRAIN_THREADS, 4);
    maxIters = conf.getInt(CVB0Config.MAX_ITERATIONS_PER_DOC, 10);
    float modelWeight = conf.getFloat(CVB0Config.MODEL_WEIGHT, 1.0f);
    testFraction = conf.getFloat(CVB0Config.TEST_SET_FRACTION, 0.1f);
